...
def calculate_energy_magnetization(lattice, J=1):
    """Calculate the energy and magnetization of a given Ising lattice configuration."""
    L = lattice.shape[0]
    energy = 0
...
def gray_code_generator(L):
    """Generate configurations one at a time using a generator for an LxL lattice."""
    N = L * L
...
# Compute the histogram of (E, M) configurations using chunked processing
...
# Compute temperature-dependent probability distribution Pi_M(T)
def compute_probability_distribution(E_M_histogram, T):
    """Compute the temperature-dependent probability distribution pi_M(T) for a given temperature."""
...
def compute_binder_cumulant(probability_distribution):
    """Calculate the Binder cumulant B(T) based on the probability distribution pi_m."""
    m2 = sum([M**2 * prob for M, prob in probability_distribution.items()])
    m4 = sum([M**4 * prob for M, prob in probability_distribution.items()])
    return 1 - (m4 / (3 * m2**2))

# Plot Probability Distribution π_M vs M for the given lattice size and temperature
...
def main():
    ...
    # Show the single Binder Cumulant plot
    plt.show()
